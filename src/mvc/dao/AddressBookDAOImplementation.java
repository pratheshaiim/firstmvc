...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into address values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"','"+person.getAddress()+"')";
    ...
    @Override
     public void removePerson(String name)
     {
         try
         {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
           String qry="delete from address where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
         }
         catch (SQLException e)
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update address set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"',Address='"+person.getAddress()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    public DefaultListModel getAllNames(NameListPanel namePanel) 
    {
         ...
         conn = DBConnection.getInstance().getConnect();
            ...
            String qry = "select * from address order by name asc";
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            Statement stmt;
            stmt = conn.createStatement();
            String qry = "select * from address where name = '"+selectedName+"'";
            ResultSet rs;
